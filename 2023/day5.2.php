...
kgwynn@CHGGZRQBK3:~/dev/advent-of-code/2023$ (main) php day5.2.php < ../../advent-of-code-inputs/2023/day5.input.txt
...
// Loop on each seed + range pair
for ($i = 0; $i < count($seeds); $i += 2) {

    $seed_base = intval($seeds[$i]);
    $seed_range = intval($seeds[$i + 1]);

    // loop on each seed value within range
    for ($seed = $seed_base; $seed < $seed_base + $seed_range; $seed++) {
        $target = intval($seed);
